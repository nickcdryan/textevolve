...
class AgentSystem:
    ...
    def _load_previous_state(self):
        ...
        if summaries:
            ...
            prompt = f"""
        You are tasked with creating a Python script that solves grid transformation problems as accurately as possible.

        The provided script should load grid data from a structured text format.  You will receive a 'question' containing an input grid and expected output as training example(s). The goal is to transform a 'test_input' grid following the same pattern inferred from training. All grids will be represented in the following JSON format: [[row1], [row2], ...]. Ensure your script handles various grid sizes and transformation logic correctly.  Assume the text input follows the standard field names.

        Available functions:

        ```python
        def call_llm(prompt, system_instruction=None):
            # Calls an LLM with the given prompt (string format) and an optional system instruction string. Returns an LLM response string.
            pass

        def extract_answer(text):
            # Extracts the answer part from the given text
            pass

        def create_json_grid(grid):
            # Converts a grid (list of lists) to a JSON string representation
            pass

        def load_grid(grid_json):
            # Loads a grid from a JSON string representation. Returns a list of lists.
            pass

        def analyze_grid_transformation(input_grid, output_grid):
            # Analyzes the transformation between two grids and returns a description of the changes.
            prompt = f'''
            Analyze the transformation applied to the input grid to produce the output grid.

            Input Grid: {create_json_grid(input_grid)}
            Output Grid: {create_json_grid(output_grid)}

            Provide a JSON object representing the transformation rules.  The JSON should have a "rules" field, which is a list of transformation rule objects.  Each rule object can have these properties:
            1. "type": The type of transformation (e.g., "replace"). Currently, only "replace" is supported.
            2. "value": The new value to use in the transformation.
            3. "positions": A list of [row, col] pairs indicating the positions in the grid where the rule applies.
            ...
            ```
            '''
            return call_llm(prompt)  # Call LLM to analyze the transformations
        


        def apply_transformation(input_grid, transformation_description):
            # Applies the described transformation to the input grid and returns the resulting grid.
            try:
                transformation_rules = json.loads(transformation_description)
                output_grid = [row[:] for row in input_grid]  # Create a copy to modify

                for rule in transformation_rules.get("rules", []):
                    if rule["type"] == "replace":
                        for pos in rule.get("positions", []):
                            row, col = pos
                            if 0 <= row < len(output_grid) and 0 <= col < len(output_grid[0]):
                                output_grid[row][col] = rule["value"]

                return output_grid

            except Exception as e:
                print(f"Error applying transformation: {e}")
                # Fallback: Call the LLM if structured transformation fails

                prompt = f'''Apply the following transformation to the input grid:

                Transformation Description: {transformation_description}
                Input Grid: {create_json_grid(input_grid)}

                Return ONLY the transformed grid in JSON format: [[row1], [row2], ...]
                '''
                transformed_grid_json = call_llm(prompt)
                return load_grid(transformed_grid_json)  # Returns transformed grid


        ```

        Here are some examples of recent successes and failures, along with the approach descriptions:

        {past_performance_examples}

        Here's the current accumulated learnings about this dataset and task:
        ```
        {self.learnings}
        ```

        Now, create a Python script that effectively solves this type of grid transformation problem for any unseen example by leveraging learnings from this dataset and task.  Focus on generating code that accurately identifies and replicates the transformation shown in the training examples.  The generated script MUST use the given helper functions appropriately.

        Your generated script MUST use the given helper functions appropriately.
        Begin your script with the following function definition:

        ```python
        def solve(question):
            # Load training examples and test input
            # Your transformation logic here using the provided helper functions
    ...
    def synthesize_learnings(self, current_learnings: str, new_batch_learnings: str) -> str:
        ...
        system_instruction = "You are a Knowledge Integrator. Your role is to synthesize accumulated dataset-specific knowledge with new insights, creating an evolving experiment log that captures concrete patterns, strategies, and findings about this specific task."
                ...
                # Base prompt content
        base_prompt = f"""
        You are tasked with synthesizing existing knowledge with new learnings from our latest experiment on this specific grid transformation dataset.
        ...
        Synthesize these learnings into a structured format that prioritizes concrete, dataset-specific insights over general system design principles.  Your output should resemble a detailed research log specifically focused on the grid transformation task.

        Organize the synthesized learnings into these sections, preserving specific examples and findings:

        # Grid Transformation Task: Dataset-Specific Learning Log

        ## 1. DATASET PATTERNS & CHARACTERISTICS:
        * Focus on observed patterns in grid structure, numerical values, transformation types, and any unique characteristics.
        * Examples: Grid size variance, numerical value distributions, transformation complexity, symmetry considerations, potential size changes.

        ## 2. EFFECTIVE TASK-SPECIFIC STRATEGIES:
        * Describe specific techniques that have shown promise in solving grid transformations on this dataset.
        * Examples: Localized transformation identification, conditional logic based on neighbor values, handling specific numerical values (like '0' as background). If no strategy has been effective yet, clearly state that.

        ## 3. COMMON FAILURE MODES ON THIS DATASET:
        * Analyze specific ways solutions have failed on this dataset and why. Relate failure modes to dataset patterns and challenges.
        * Examples: LLM inability to generalize patterns, issues with spatial reasoning or handling edge cases, specific error patterns in generated grids.

        ## 4. EXPERIMENT LOG & FINDINGS:
        * Keep a concise record of each experiment: Approach summary, accuracy, key findings, primary issue identified, examples of errors, and hypotheses tested.

        ## 5. NEXT RESEARCH DIRECTIONS (Optional):  If applicable, given the latest results, briefly suggest what research directions to explore in future iterations to overcome the challenges.

        Your synthesized output should be concise, actionable, and directly relevant to the grid transformation task. Maintain a focus on concrete, dataset-specific observations and strategies.  Preserve all concrete, specific insights from both the existing learnings and new batch.  Don't lose valuable information.  Preserve the details of runtime, execution, error, and processing problems.
        """
    ...
    def adjust_explore_exploit_with_llm(self) -> Tuple[int, int]:
        ...
        try:
            ...
            # In case of error, use a more conservative adaptive heuristic
            if len(summaries) >= 3:
                ...
                if all_accuracies:
                    ...
                    accuracy_range = max_accuracy - min_accuracy if max_accuracy != min_accuracy else 0.001  # avoid div by zero

                    # Scale recent accuracy improvement/decline to 0-1 range within overall accuracy range seen so far, then scale to percentage point adjustment for explore/exploit
                    accuracy_improvement = (recent_accuracies[0] - min_accuracy) / accuracy_range
                    adjustment = int(accuracy_improvement * 20) # Max adjustment +/- 20%

                    if recent_accuracies[0] > np.mean(recent_accuracies): # improving trend favors exploitation
                        new_explore = max(10, self.explore_rate - adjustment)
                        new_exploit = 100 - new_explore
                    else:
                        new_explore = min(90, self.explore_rate + adjustment)  # declining trend favors exploration
                        new_exploit = 100 - new_explore

                    print(f"Fallback: Adjusting explore/exploit to {new_explore}/{new_exploit} based on recent trend.")
                    return new_explore, new_exploit
                else:
                    print("Fallback: No accuracies available, maintaining current balance.")
                    return self.explore_rate, self.exploit_rate

            # Default fallback: slight exploration bias in early stages
            if len(summaries) < 5:
                new_explore = min(self.explore_rate + 5, 70)
                new_exploit = 100 - new_explore
                print(f"Fallback: Early stages, adjusting explore/exploit to {new_explore}/{new_exploit}.")
                return new_explore, new_exploit
            else:
