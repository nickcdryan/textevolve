...
def is_valid_time(question, proposed_solution):
    """Implement a deterministic check for time validity."""
    system_instruction = "You are an expert in verifying the proposed solution is valid."
    prompt = f"""
    Check that the proposed solution: {proposed_solution} is valid, given the context of the question.
    Is the time in the constraints provided in the question?
    Return VALID or INVALID and include a brief reason.
    Question: {question}
...
def main(question):
    ...
    try:
        ...
        # 5. Validate time
        time_validation = is_valid_time(question, solution)

        if "VALID" in verification and "VALID" in time_validation:
