...
def main(question):
    ...
    try:
        ...
        extraction_result = extract_information(clarified_question)
...
def extract_information(question, max_attempts=3):
    ...
    for attempt in range(max_attempts):
        extraction_prompt = f"""
        Extract the information needed to answer the following question.

        Example:
        Question: What was the total number of yards gained by all players mentioned in the passage?
        Extracted Information: [Relevant sentences from the passage containing yardage information.]
...
def synthesize_answer(question, extracted_info):
    ...
    system_instruction = "You are an expert at synthesizing answers."
    synthesis_prompt = f"""
